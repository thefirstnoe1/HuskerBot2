...
class GamedayOnTest {
    ...
    fun `metadata methods return expected values`() {
        val cmd = GamedayOn()
        assertEquals("on", cmd.getCommandKey())
        assertEquals(true, cmd.isSubcommand())
        assertEquals("Turns gameday mode on", cmd.getDescription())
    }
    ...
    fun `execute replies with success embed when setGameday succeeds`() {
        ...
        assertEquals("Gameday Mode Enabled!", embed.title)
        assertEquals("Make your way to the gameday channels.", embed.description)
    ...
    fun `execute replies with ephemeral error when setGameday throws`() {
        ...
        // Message should start with the error preface used in GamedayOn
        // and contain the thrown message from setGameday (server requirement)
        // Exact message includes the exception text appended
        // We assert the prefix to avoid brittleness and ensure context is present
        assertEquals(true, msg.startsWith("Error while trying to enable gameday mode:"))
    }
}
